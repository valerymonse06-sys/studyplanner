...
export default function App() {
  const today = new Date();
    ...
    // ── State y almacenamiento persistente ────────────────────────────
  //
  // Antes usábamos window.storage, que pertenece al entorno
  // de Claude Artifacts.
  //
  // Ahora usamos localStorage, que funciona en una aplicación
  // web normal publicada en Vercel.
  //
  // Los datos quedan guardados en el navegador del dispositivo.
  ...
  useEffect(() => {
    try {
      ...
      if (savedMaterias) {
        setMateriasRaw(JSON.parse(savedMaterias));
      }

      if (savedTasks) {
        setTasksRaw(JSON.parse(savedTasks));
      }

      if (savedNotes) {
        setNotesRaw(JSON.parse(savedNotes));
      }

      if (savedNextIds) {
        const ids = JSON.parse(savedNextIds);

        if (typeof ids.mat === "number") {
          setNextMatId(ids.mat);
        }

        if (typeof ids.tar === "number") {
          setNextTarId(ids.tar);
        }

        if (typeof ids.note === "number") {
          setNextNoteId(ids.note);
        }
      }
    } catch (error) {
  ...
  // ── Setters que actualizan pantalla + almacenamiento ─────────────
  ...
  // ── Guardar los siguientes IDs ──────────────────────────────────

  useEffect(() => {
    if (!ready) return;

    persist("studyplanner_nextIds", {
      mat: nextMatId,
      tar: nextTarId,
      note: nextNoteId,
    });
